...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($experience)) { array_push($errors,"Job experience is required"); }
  if (empty($identity)) { array_push($errors,"Identity is required"); }
  if (empty($media)) { array_push($errors,"Social media is required"); }
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO users (name, email, password,contact,jobdescription,experience,identity,media) 
  			  VALUES('$name', '$email', '$password','$contact','$jobdescription','$experience','$identity','$media')";
